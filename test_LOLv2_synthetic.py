...
parser.add_argument('-opt', type=str, required=False, help='Path to options YMAL file.',default='./options/test/LOLv2_synthetic.yml')
...
def main():
    ...
    logging.info('Recovering from %s ...' % (opt['path']['pretrain_model_G']))
    checkpoint = torch.load(opt['path']['pretrain_model_G'])
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        for val_data in val_loader:
            ...
            input_img = util.tensor2img(val_data['LQs'])
            if save_imgs:
                try:
